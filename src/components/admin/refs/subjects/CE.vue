...
<script>
    ...
    export default {
        ...
        methods: {
            fetch() {
                if (this.id) {
                    this.data = _.pick(_.find(this.$store.state.subjects, {id: this.id}) || {}, ['name', 'ord']);
                } else {
                    this.data = emptyData();
                }
                this.loading = false;
            ...
            onSubmit(e) {
                ...
                if (this.id) {
                    req = ajax.reqAPI(`dic/subjects/${this.id}`, {method: 'PUT', body});
                } else {
                    req = ajax.reqAPI(`dic/subjects`, {method: 'POST', body});
                }
                req.then(() => this.$store.dispatch('reloadSubjects')).then(() => {
                    this.$router.back();
                    this.loading = false;
                }).catch(error => {
                    if (error.status === 401) {
